...
/**
 * rev_str - function that reverses the order of a string
 *
 * Description:
...
/**
 * add_str - adds two strings and returns the result
 *
 * Description:
...
*/
char *add_str(char *sum, const char *num1, const char *num2)
{
...
int main(void)
{
	...
	for (i = 2; i < 100; i++)
	{
		printf("%s%s", add_str(fi[2], fi[1], fi[0]), (i != 99) ? ", " : "\n");
		strcpy(fi[0], fi[1]);
		strcpy(fi[1], fi[2]);
	}
